...
if (loginForm) {
    ...
    loginForm.addEventListener(
        ...
        async function (event) {
            ...
            // ==================================
            // LOCAL STUDENT LOGIN CHECK
            // ==================================
            const studentAccounts = JSON.parse(localStorage.getItem('studentAccounts')) || [];
            const foundStudent = studentAccounts.find(s => s.email.toLowerCase() === email && s.password === password);

            if (foundStudent) {
                localStorage.setItem("userRole", "student");
                localStorage.setItem("userEmail", foundStudent.email);
                localStorage.setItem("studentLoggedIn", "true");
                localStorage.setItem("currentStudent", JSON.stringify(foundStudent));

                showMessage("تم تسجيل الدخول كطالب بنجاح 🎉", "success");
                setTimeout(function () {
